...
///
/// On launch: locate Docker → ensure the `ticktalk-engine:dev` image exists
...
final class DockerBackendController {
    ...
    /// UI observes the same controller (see TickTalkApp / RootSplitView).
    ...
    private let imageTag = "ticktalk-engine:dev"
