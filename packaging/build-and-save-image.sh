...
#
# Dev: the tar lands in macos/TickTalk/.build (gitignored). The Xcode app
# target's build phase copies it into TickTalk.app/Contents/Resources, where
...
#   DESK_IMAGE_TAR=<abs path to engine-image.tar.gz> open .build/TickTalk.app
set -euo pipefail

cd "$(dirname "$0")/.."          # repo root
TAG="${1:-ticktalk-engine:dev}"
OUT="${2:-macos/TickTalk/.build/engine-image.tar.gz}"
