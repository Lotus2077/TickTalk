# TickTalk — native macOS app for TradingAgents

TickTalk is a native macOS app (SwiftUI, macOS 14+, built with Xcode 26 /
...
![TickTalk — the workspace after a completed NVDA run: the Watchlist with live decisions, the per-ticker Journal, and the Live Monitor's timeline + Agent Theater.](images/ticktalk.png)
...
| `macos/TickTalk/` | The SwiftUI app (SwiftPM package) + dev build/sign scripts. |
...
- **What.** Creates a stable self-signed **"TickTalk Dev"** code-signing
...
bash macos/TickTalk/scripts/dev-signing-setup.sh
...
bash macos/TickTalk/scripts/make-preview-app.sh
open macos/TickTalk/.build/TickTalk.app
# (or just `swift build` / `swift run` from macos/TickTalk/)
```
