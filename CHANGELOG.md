...
All notable changes to TickTalk and its bundled TradingAgents engine are
documented here.
...
First public release of **TickTalk** — a native macOS app for the TradingAgents
engine — as a standalone repository. API keys are stored in the macOS Keychain
under the service `io.github.lotus2077.ticktalk` and injected into the backend
per run. Bundled engine: TradingAgents v0.3.0 (see prior entries).
...
- **TickTalk backend (`desk_server/`, `desk_adapter/`).** A FastAPI + SSE
...
- **TickTalk — native macOS app (`macos/TickTalk/`).** A SwiftUI app
  ...
  [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Engine graph unchanged.
